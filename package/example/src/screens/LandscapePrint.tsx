...
type LandscapePrintRouteProp = RouteProp<RootStackParamList, 'LandscapePrint'>;

export const LandscapePrint = memo(() => {
  const {
    params: { printer },
  } = useRoute<LandscapePrintRouteProp>();
  ...
  const printSimpleReceipt = async () => {
    try {
      ...
      const res = await printerInstance.addQueueTask(async () => {
        ...
        await printerInstance.addPageArea(0, 0, 636, 636);

        await printerInstance.addPageDirection(PageDirectionType.DIRECTION_BOTTOM_TO_TOP);

        await printerInstance.addPagePosition(0, 60);
  ...
  return (
    <View style={styles.container}>
      <View style={styles.contentCotainer}>
        <ScreenTitle title={'Landscape Print'} />
      </View>
      <View style={styles.contentCotainer}>
